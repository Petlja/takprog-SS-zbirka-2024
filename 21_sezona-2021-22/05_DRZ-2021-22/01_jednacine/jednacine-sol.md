...
S(n) = S(A\cdot T + B) = T
$$
и ако је неки од њих задовољава, исписати његову вредност, у супротном исписати број $-1$. Број елемената скупа $U$ је $O(\log Y)$, а провера да ли неки елемент $n$ скупа $U$ задовољава горњу једнакост има сложеност $O(\log n)=O(\log Y)$, па је сложеност целог алгоритма $O(\log^2 Y)$. 
